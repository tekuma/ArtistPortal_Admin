...
public class TekumaAction extends BaseAction {
    ...
    public String findcollwnlist(){
		...
		this.pageResult=this.tekumaService.findcollwnlist(this.pageResult,collcategories,collstyles,collsubjects,color);
		...
		for(int i=0;i<path.size();i++){
			picurlr= path.get(i);
			if(picurlr.getStoreaddress()!=null&&picurlr.getStoreaddress()!=""){
	...
	public String findCollectionlist(){
		...
		this.pageResult.setPageNo(Integer.valueOf(pageNo));
		this.pageResult.setPageSize(14);
		this.pageResult=this.tekumaService.findCollectionlist(this.pageResult,memberid);
		String sql="select avatarpath,lastname,firstname,birthday,telephone,email,introduction,bank,bankaccount,payee from tk_member_t where ID="+memberid;
		this.memberNew=(Member)this.tekumaService.getPsychologyJdbcDao().getList(sql, Member.class).get(0);
	...
	public String findPoollist(){
		...
		this.pageResult=this.tekumaService.findPoollist(this.pageResult,collectionid);
	...
	/*	String sql="select * from tk_collection_t where ID="+collectionid;
		this.collectionNew=(Collection)this.tekumaService.getPsychologyJdbcDao().getList(sql, Collection.class).get(0);*/
		String SQL="select * from tk_display_t where CID="+collectionid+" order by Time desc limit 0,2 ";
		this.displaylist=this.tekumaService.getPsychologyJdbcDao().getList(SQL, Display.class);
		//显示缩略图
	...
	public void finddisplayidd(){
		String SQL="select * from tk_display_t where ID="+this.displayNew.getId();
		this.displayNew=(Display)this.tekumaService.getPsychologyJdbcDao().getList(SQL, Display.class).get(0);
		this.entityReturnJson(this.displayNew);
	}
	
	//根据Collectio_id查询出Collectio所有展览信息
		public void finddisplaycid(){
			String SQL="select * from tk_display_t where CID="+this.displayNew.getCid()+" order by Time desc";
			this.displaylist=this.tekumaService.getPsychologyJdbcDao().getList(SQL, Display.class);
	...
	public void updatedispaly(){
		String SQL="select * from tk_display_t where ID="+this.displayNew.getId();
		int count=this.tekumaService.getPsychologyJdbcDao().getTempInt(SQL, 0);
		if(count!=0){
			this.tekumaService.getPsychologyJdbcDao().executeMYSQLDB("update", "tk_display_t", this.displayNew, "id");
			this.entityReturnJson("Update successd！");
		}else{
			this.tekumaService.getPsychologyJdbcDao().executeMYSQLDB("save", "tk_display_t", this.displayNew, "");
	...
	public void deletedispaly(){
		this.tekumaService.getPsychologyJdbcDao().executeMYSQLDB("delete", "tk_display_t", this.displayNew, "id");
		this.entityReturnJson("Delete successd!");
	}
	
	//根据Collectio_id查询第二个之后的所有展览信息
	public void finddisplaylist(){
		String SQL="select count(1) from tk_display_t where CID="+this.displayNew.getCid()+" order by Time desc";
		int count=this.tekumaService.getPsychologyJdbcDao().getTempInt(SQL, 0);
		String sql="select * from tk_display_t where CID="+this.displayNew.getCid()+" order by Time desc limit 2,"+count+"";
		this.displaylist=this.tekumaService.getPsychologyJdbcDao().getList(sql, Display.class);
		this.listReturnJson(this.displaylist);
	}
