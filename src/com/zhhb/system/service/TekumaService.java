...
public class TekumaService {
	...
	public User findUser(User userNew) {
		String sql = "select UserName from tk_user_t where LoginName='"
				+ userNew.getLoginname() + "' and LoginPwd='"
				+ userNew.getLoginpwd() + "'";
	 ...
	 */
	/*
	 * public List<Member> findMemberType(){ String
	 * sql="select MemberType from tk_member_t group by MemberType"; return
	 * this.psychologyJdbcDao.getList(sql,Member.class); }
	 */
	 ...
	 * @return
	 */
	public PageResult findcollwnlist(PageResult pageResult,
			String collcategories, String collstyles, String collsubjects,
			String color) {
		...
		sql += "group by t1.ID ";
		pageResult = this.getPsychologyJdbcDao().findPageResult(sql,
				pageResult, Collection.class);
		return pageResult;
	}

	/**
	 * 分页查询出所有会员 Check out all of the members
	 * 
	 * @return aa
	 */
	public PageResult findMemberlist(PageResult pageResult, String letters,
			String categories, String styles, String subjects) {
		...
		sql += "group by t1.id";
		pageResult = this.getPsychologyJdbcDao().findPageResult(sql,
				pageResult, Member.class);
		return pageResult;
	}

	/**
	 * 根据会员id查询所有Collection文件 Query all Collection files according to member ID
	 * 
	 * @return
	 */
	...
	/**
	 * 根据Collectionid查询所有pool文件 Query all pool files according to member
	 * Collectionid
	 * 
	 * @return ID,MemberID,Title,CreateTime,EntryLabel,Description,UploadTime,
	 *         storeaddress
	 */
	public PageResult findPoollist(PageResult pageResult, String Collectionid) {
		String sql = "select t1.COLLECTIONID,t2.ID,t2.MemberID,t2.Title,t2.CreateTime,t2.storeaddress,t3.CollectionTitle,t3.DescriptionOf,t3.PrintQuantity from tk_relevancy_t t1 "
				...
				+ Collectionid
				+ "' order by t2.UploadTime desc";
		pageResult = this.getPsychologyJdbcDao().findPageResult(sql,
				pageResult, Pool.class);
		return pageResult;
	}
